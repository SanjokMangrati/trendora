...
export function CustomersTable() {
    ...
    const canGenerate = (customer: Customer) => {
        const orderCount = customer.Session.reduce((sum, session) => sum + session.orders.length, 0);
                            ...
                            <TableRow key={customer.id}>
                                ...
                                <TableCell>
                                    <Button
                                        ...
                                        disabled={!canGenerate(customer) || generatingIds.includes(customer.id)}
                                    >
                                        {generatingIds.includes(customer.id) ? <Loader size='xs' /> : 'Generate'}
                                    </Button>
                                </TableCell>
                            </TableRow>
                        ))}
